"""
noise_psd_fits_90ghz.py
Compute BB noise pseudo-Cl from sign-flip 90GHz FITS maps.
Run from terminal: python noise_psd_fits_90ghz.py
"""

# cd ~/Initial_test/Code/Map_Operation

# nohup python -u noise_psd_fits_90ghz.py > noise_psd_fits_90ghz.log 2>&1 &

# tail -f noise_psd_fits_90ghz.log
...
N_FILES      = 10
# The 90 GHz FITS products are stored with a 095 GHz filename suffix.
all_sf_files = sorted(glob.glob(os.path.join(SIGNFLIP_DIR, "signflip_noise_permutation*_095ghz.fits")))
...
LMAX     = 3000
OUT_FILE = os.path.expanduser("~/Initial_test/outputs/noise_psd_BB_fits_90ghz.npz")
...
def main():
    ...
    if not SIGNFLIP_FILES:
        raise RuntimeError(
            f"No sign-flip FITS files matched {SIGNFLIP_DIR!r} for the 90 GHz band. "
            "Expected filenames with a 095ghz suffix."
        )
